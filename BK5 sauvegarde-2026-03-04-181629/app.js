...
function renderDepartureSuggestion(result) {
    ...
    container.innerHTML =
        ...
        `Vent départ: ${depWind} · Vent arrivée: ${arrWind} · Vent min trajet: ${minWind} · Vent max trajet: ${maxWind}`;
}
...
async function suggestBestDeparture() {
    ...
    try {
        ...
        const safeCandidates = candidates.filter(c => c.isSafe && c.isNoMotor);
        ...
        if (!best) {
            ...
            if (container) {
                container.classList.remove('suggestion-clickable');
                container.textContent = 'Suggestion départ: aucune fenêtre avec vent > 5 kn et ≤ 20 kn trouvée.';
            }
            return;
        }
...
async function computeRoute() {
    ...
    for (let i = 0; i < routePoints.length - 1; i++) {
        ...
        let safeLegPoints = avoidLandIsActive
        ...
        safeLegPoints = densifyPolylineForWeather(safeLegPoints, maxDistanceForWeatherNm);

        if (safeLegPoints.length > 2) {
